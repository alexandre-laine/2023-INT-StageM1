...
def make_clustering():
    ...
    for icluster, cluster in enumerate(prm.cluster_list) :
        if cluster == 'Steven_AA003_cl0' : # this is a MUA cluster TODO REMOVE
            ...
            try :
                ...
                kmeans_data[icluster, 1] = [x['last_bt'] for x in kmeans_arr_last_bt if x['cluster'] == cluster][0] # safe
                kmeans_data[icluster, 2] = [x['cv'][1]['c50'] for x in kmeans_arr_clustering_array if x['cluster'] == cluster][0] # unsafe
                kmeans_data[icluster, 3] = [x['cv'][1]['b'] for x in kmeans_arr_clustering_array if x['cluster'] == cluster][0] # unsafe
                kmeans_data[icluster, 4] = [x['cv'][1]['n'] for x in kmeans_arr_clustering_array if x['cluster'] == cluster][0] # unsafe
                kmeans_data[icluster, 4] = np.log(kmeans_data[icluster, 4])
                kmeans_data[icluster, 5] = [x['cv'][0][0]/x['cv'][0][-1] for x in kmeans_arr_clustering_array if x['cluster'] == cluster][0] # safe-ish
                
                kmeans_data[icluster, 6] = [x[1][0] for x in kmeans_arr_optimal_delay if x[0] == cluster][0] # safe
                kmeans_data[icluster, 7] = [x[1][-1] for x in kmeans_arr_optimal_delay if x[0] == cluster][0] # safe
                
                kmeans_data[icluster, 8] = [x['early_late_ratios'][0] for x in kmeans_arr_early_late if x['cluster'] == cluster][0] # safe
                kmeans_data[icluster, 9] = [x['early_late_ratios'][-1] for x in kmeans_arr_early_late if x['cluster'] == cluster][0] # safe
    ...
    '''for icluster, cluster in enumerate(prm.cluster_list) :
        try :
            print(np.abs(kmeans_data[icluster, 2] - [x['nkr_params']['b'] for x in extra_arr if x['cluster'] == cluster]))
